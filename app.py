...
IMAGENES_CAPAS = {
    "Vista General": "vista General.gif",
    "Capas": "tierra.png",
...
# --- BANCO DE PREGUNTAS (Editable como texto plano) ---
CUESTIONARIO = {
    "general": [
        {
            "id": "g1",
...
with tab_general:
    ...
    # Render modular del cuestionario
    st.write("---")
    with st.expander("📝 Pon a prueba tus conocimientos sobre Nuestro Planeta"):
        for q in CUESTIONARIO["general"]:
            ...
            if st.button("Validar Respuesta", key=f"btn_{q['id']}"):
                if ans == q["correcta"]: st.success("🎉 ¡Excelente! Respuesta correcta.")
                else: st.error(f"❌ Incorrecto. Pista: {q['pista']}")
...
with tab_hidro:
    ...
    with col2:
        ...
        st.markdown("• **3% Agua Dulce:** Ríos, lagos, hielo y agua subterránea que sustentan la vida terrestre.")
        st.markdown("• **75% Del planeta tierra está cubierto por agua.**")
